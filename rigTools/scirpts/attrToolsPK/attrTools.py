...
class AttrToolsClass(object):
        ...
        if ctrl != None and mc.objExists(ctrl):
            if attrList:
                ...
                for attr in attrList:
                    infoDict = dict()
                    infoDict[attr] = {}
                    exitAttr = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], ex=1)
                    if exitAttr and (mc.getAttr(list(attrInfoDict.keys())[-1] + '.' + attr, cb=1) or mc.getAttr(
                            list(attrInfoDict.keys())[-1] + '.' + attr, k=1)):
                        infoDict[attr]['type'] = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], at=1)
                        if infoDict[attr]['type'] == 'typed':
                            continue
                        infoDict[attr]['longName'] = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], ln=1)
                        infoDict[attr]['key'] = mc.getAttr(list(attrInfoDict.keys())[-1] + '.' + attr, k=1)
                        infoDict[attr]['cb'] = mc.getAttr(list(attrInfoDict.keys())[-1] + '.' + attr, cb=1)
                        infoDict[attr]['lock'] = mc.getAttr(list(attrInfoDict.keys())[-1] + '.' + attr, l=1)
                        infoDict[attr]['niceName']= mc.addAttr(list(attrInfoDict.keys())[-1] + '.' + attr,nn =1,q =1)
                        if infoDict[attr]['type'] != 'enum':
                            hasMinValue = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], mne=1)
                            hasMaxValue = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], mxe=1)
                            if hasMinValue:
                                infoDict[attr]['min'] = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], min=1)[0]
                            else:
                                infoDict[attr]['min'] = None
                            if hasMaxValue:
                                infoDict[attr]['max'] = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], max=1)[0]
                            else:
                                infoDict[attr]['max'] = None
                            infoDict[attr]['defaultValue'] = \
                            mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], ld=1)[0]
                        else:
                            infoDict[attr]['enum'] = mc.attributeQuery(attr, node=list(attrInfoDict.keys())[-1], le=1)[0]
    ...
    def createAttr(self,ctrl = '',attrInfoList = []):
        if attrInfoList and mc.objExists(ctrl):
            for attrInfo in attrInfoList[0]:
                for attr,info in list(attrInfo.items()):
    ...
    def setDefaultsBySelect(ctrlList = [],attrList = [],key =1,lock =0,u =1,v =1):
        channelsAttrList = AttrToolsClass.getSelectedChannels(ctrlList[-1], key=key, lock=lock,
                                                              u=u, v=v)
