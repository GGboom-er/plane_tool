    ...
    def geoInfo( self, vtx=0, geo=0, shape=0, skinC=0 ):  # Returns a list of requested object strings
        ...
        if vtx == 1:
            if len(selVTX) != 0:  # if vertices are already selected, then we can take that list whole-sale.
                ...
                vtxIndexList = ["{0}.vtx[{1}]".format(geoShape, x) for x in
                                cmds.getAttr(geoShape + ".vrts", multiIndices=True)]
...
def copySkinWeightsUV( _ ):
    ...
    mel.eval(
        'copySkinWeights -noMirror -surfaceAssociation closestPoint -uvSpace UVChannel_1 UVChannel_1 -influenceAssociation closestJoint -influenceAssociation oneToOne -influenceAssociation oneToOne;')
...
def RevUI( *args ):
    ...
    cmds.text(label="TRANSFER RIGGED SKELETON TO SCALED OR MODIFIED MESH", width=450, height=50, align="center",
              fn="boldLabelFont")
    ...
    headmeshTXT = cmds.textField("headmeshTXT", backgroundColor=[0.1, 0.1, 0.1], editable=False, width=300, height=28,
                                 p="row_selectmesh1")
    cmds.button("selectHeadBtn", label="<<    Original Mesh", width=150, height=26,
                command=button_wrapper(store_og, headmeshTXT))

    # each of the select mesh rows
    cmds.rowLayout("row_selectmesh2", numberOfColumns=2, parent="col_selectmesh")
    teethmeshTXT = cmds.textField("teethmeshTXT", backgroundColor=[0.1, 0.1, 0.1], editable=False, width=300, height=28,
                                  p="row_selectmesh2")
    cmds.button("selectTeethBtn", label="<<    Modified Mesh", width=150, height=26,
                command=button_wrapper(store_new, teethmeshTXT))

    # each of the select mesh rows
    cmds.rowLayout("row_selectmesh3", numberOfColumns=2, parent="col_selectmesh")
    teethmeshTXT = cmds.textField("jointTXT", backgroundColor=[0.1, 0.1, 0.1], editable=False, width=300, height=28,
                                  p="row_selectmesh3")
    cmds.button("selectJointBtn", label="<<    Root Joint", width=150, height=26,
                command=button_wrapper(store_joint, teethmeshTXT))
    ...
    cmds.button(label="UV Method", backgroundColor=[0.80, 0.65, 0.0], align="left", width=225, height=26,
                command=copySkinWeightsUV)
    cmds.button(label="Dictionary Method (Slow)", backgroundColor=[0.80, 0.65, 0.0], align="center", width=225,
                height=26, command=exportWeightsdiceat)

    cmds.columnLayout("col_note", parent="AllLayout")
    cmds.text(label="              ", width=450, height=30, align="left")
    cmds.text(label=" NOTE: The old mesh and new mesh should have the same topology and vertex ID", width=450,
              height=30, align="left", bgc=[0.79, 0.45, 0.45])
    cmds.text(label="              ", width=450, height=5, align="left")
    cmds.text(label=" UV METHOD: Quick but needs same UV (same sets as well) without overlap", width=450, height=30,
              align="left", bgc=[0.79, 0.45, 0.45])
    cmds.text(label="              ", width=450, height=5, align="left")
    cmds.text(label=" DICTIONARY METHOD: Very SLOW but accurate, independent of UV  (RECOMMENDED)", width=450,
              height=30, align="left", bgc=[0.79, 0.45, 0.45])
    cmds.text(label="              ", width=450, height=5, align="left")
